...
### 17. Interactive API Documentation (OpenAPI/Swagger)
**Before:** No self-updating documentation; a custom HTML explorer required manual endpoint duplication and quickly became stale.

**After:** Standards-based OpenAPI docs generated by `swag` and served through Swagger UI.
```go
// main.go
import (
...
**Key updates:**
- Added `github.com/swaggo/gin-swagger`, `github.com/swaggo/files`, and the generated `docs` package to `go.mod`.
- Annotated every public handler (`routes/users_api.go`, `routes/event_api.go`, `routes/register_api.go`) with `@Summary`, `@Tags`, `@Param`, `@Success`, and security metadata so the spec stays in sync with the code.
- Generated OpenAPI artifacts via `/Users/sraj/go/bin/swag init --parseDependency --parseInternal -g main.go -o docs`, producing `docs/swagger.yaml`, `docs/swagger.json`, and `docs/docs.go`.
- Exposed the interactive UI at `/swagger/index.html`, replacing the hard-coded docs page with an automatically updated experience.

**Benefits:**
- Accurate, always-up-to-date docs tied directly to handler annotations.
- Consumers can try endpoints interactively, including Bearer-authenticated calls, without leaving the browser.
- Spec files enable client SDK generation, contract testing, and publishing to portals.
- Regeneration is a single command whenever routes or models change, reducing drift and manual effort.
...
5. **Docs Automation** - Automate Swagger generation/validation in CI and publish versioned docs
